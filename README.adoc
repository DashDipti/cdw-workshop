...
=== 3.1 Open Hue for CDW Virtual Warehouse - `emeaworkshop-hive` +

- Click on the image:images/step3/hue.png[] button on the right upper corner of `emeaworkshop-hive` as shown in the screenshot below. +
image:images/step3/31-1-Replace.png[] +
...
=== 3.2 Run the following DDL in HUE for the CDW Virtual Warehouse - `emeaworkshop-hive` +
...
=== 3.3 Run the following DDL in HUE for the CDW Virtual Warehouse - `emeaworkshop-impala`. +

- Go to the page where now you will access HUE of an Impala virtual warehouse. Click on `HUE` for *`emeaworkshop-impala`* as shown in the screenshot below.
...
- Go back to the CDP Console and observe the Impala Virtual Warehouse `emeaworkshop-impala`. +
...
- Make sure that you are using the HUE of `emeaworkshop-hive`. Else, click on `HUE` and go to the HUE browser. +
...
From the cloudera console click on -  `emeaworkshop-impala`. +
...
- Open HUE for the CDW `Hive` Virtual Warehouse - `emeaworkshop-hive`
...
- Open HUE for the CDW `Impala` Virtual Warehouse - `emeaworkshop-impala`. +
...
- This can be used for both Iceberg tables and Hive Tables to improve performance. Go to the Cloudera console and look for `emeaworkshop-hive`. Click on the `Hue` button on the right upper corner of `emeaworkshop-hive` as shown in the screenshot below. +
...
- Test the policy is working - Open `HUE` for the CDW Impala Virtual Warehouse - `emeaworkshop-impala` and execute the following query. +
