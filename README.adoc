...
- Click on `Fields` column on the left window pane. The click on `EDIT FIELDS`. +
image:images/step7/31.png[] +
image:images/step7/32.png[] +

- Click on `SHOW DATA`.
image:images/step7/33showdata.png[] + 
image:images/step7/34showdata.png[] +

- Make sure that you click on the highlighted area to change `#` (measures icon) next to each column to `Dim` (dimension icon). The columns are as following. +
