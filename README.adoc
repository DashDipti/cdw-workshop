...
- Verify that youh have the joins which are as following. You can do so by clicking the image:images/step7/joinicon.png[] `join` icon.

- Click on `SHOW DATA`.
image:images/step7/30showdata-1.png[] + 
image:images/step7/30showdata-2.png[] +

- Click on `Fields` column on the left window pane. The click on `EDIT FIELDS`. Make sure that you click on the highlighted area to change `#` (measures icon) next to each column to `Dim` (dimension icon). The columns are as following. +
